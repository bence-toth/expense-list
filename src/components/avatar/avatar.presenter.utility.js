...
const generateHslColor = ({hue}) => `hsl(${hue}, 45%, 65%)` // I ❤ HSL
...
const generateBackgroundColor = ({name}) => {
  ...
  const minDistanceFromBrandColor = 25
  const forbiddenIntervalLength = 2 * minDistanceFromBrandColor
  // Chebyshev polynomials!
  const randomNumber = (parseInt(md5(name).slice(0, 4), 16)) % (360 - forbiddenIntervalLength)
